...
class Morpion:
    ...
    def create_gui(self):
        ...
        CTkButton(score_frame, text="Recomencer", command=self.new_game, width=250, fg_color="#5801c6", hover_color="#3c0384",
                  height=70, font=("Rubik bold", 20), corner_radius=64).grid(row=5+2, column=0, columnspan=2, pady=3)
        CTkButton(score_frame, text="Nouvelle partie", command=self.reset_scores, width=250, fg_color="#5801c6", hover_color="#3c0384",
                  height=70, font=("Rubik bold", 20), corner_radius=64).grid(row=6+2, column=0, columnspan=2, padx=60, pady=30)

        # création du switch

        switch = CTkSwitch(frame2, text="3x3", command=self.basculer,
                           variable=self.switch_var, onvalue="on", offvalue="off", font=("Rubik bold", 16))
        switch.pack(anchor=CENTER, pady=64, padx=64)

        # fontion pour le switch
    ...
    # foncton pour voir l'alignement des sybmoles
    ...
    def button_click(self, i, j):
        ...
        if text == " ":
            ...
            if self.check_winner():
                ...
                CTkmessagebox.showinfo("Fin de partie", "Match nul !")
                self.new_game()
            else:
